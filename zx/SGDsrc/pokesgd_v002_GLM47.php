...
class PokeSgd {
    ...
    private function applyPokesToSNA($inputFile, $outputFile, $pokeGroups, $selectedGroups, $v145EndMarker) {
    ...
    private function applyPokesToZ80v145($inputFile, $outputFile, $pokeGroups, $selectedGroups, $v145EndMarker) {
        $inputHandle = fopen($inputFile, 'rb');
        if (!$inputHandle) {
        ...
        } elseif ($this->v145EndMarker === null && $headerInfo['isCompressed']) {
            fwrite($outputHandle, "\x00\xED\xED\x00");
        }
    ...
    private function applyPokesToZ80v201v300($inputFile, $outputFile, $pokeGroups, $selectedGroups, $version) {
        ...
        while (true) {
            ...
            $uncompressedMarker = chr(0xFF) . chr(0xFF);
            
            fwrite($outputHandle, $uncompressedMarker . chr($bank));
            fwrite($outputHandle, $memory);
        }
        
        fwrite($outputHandle, "\x00\x00\xED\xED\x00");
    ...
    public function processPokes($filePath, $commandLinePokes = null, $pokelistselect = null, $v145EndMarker = null) {
        if (!file_exists($filePath)) {
        ...
        if ($extension == 'SNA') {
            $result = $this->applyPokesToSNA($filePath, $outputFile, $pokeGroups, $selectedGroups, $v145EndMarker);
        } elseif ($extension == 'Z80') {
            if ($z80Version == 145) {
                $result = $this->applyPokesToZ80v145($filePath, $outputFile, $pokeGroups, $selectedGroups, $v145EndMarker);
            } else {
...
$v145endmarker = isset($_GET['v145endmarker']) ? $_GET['v145endmarker'] : null;

$v145endmarkerCLI = null;
...
if ($POKES === null && php_sapi_name() === 'cli') {
    for ($i = 2; $i < $argc; $i++) {
        ...
        } elseif (strpos($argv[$i], 'v145endmarker=') === 0) {
            $v145endmarker = substr($argv[$i], 15);
        }
    }
}

$pokeSgd = new PokeSgd($logerrors, $v145endmarker);
$v145EndMarker = $v145endmarker;
