...
<head>
	...
	Changes:
	v008f_010726 - Fixed promotion/relegation to properly swap teams between divisions (bottom 2 swap with top 2 of lower division)
	v008f_010626 - Team-based promotion/relegation (first 2 up, last 2 down), transfer offers with accept/decline, mid-season and end-of-season transfers, performance-based offers
	v008e_010626 - Round-robin league fixture system - fixed schedule, consistent results
            ...
            weekFixtures.forEach(fixture => {
                ...
                if (home === player.currentClub || away === player.currentClub) {
                    ...
                    if (matchState.isPlaying && matchState.teamScore > 0) {
                        const result = simulateMatchWithPlayerGoals(home, away, divKey, matchState.teamScore);
                        results.push(result);
                    } else {
                        const playerResult = simulateMatch(home, away, divKey);
            ...
            teamsData.countries.forEach(country => {
                Object.keys(country.divisions).forEach(divNum => {
                    ...
                    if (promotedTeams.includes(player.currentClub) && player.country === country.name) {
                        teamPromoted = true;
                        player.division = parseInt(divNum) - 1;
