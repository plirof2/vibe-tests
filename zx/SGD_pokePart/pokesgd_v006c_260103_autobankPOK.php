<?php
/**
 * ZX Spectrum Z80/SNA POKE Patching Script v0.06
 * 
 * This script applies POKE patches to ZX Spectrum Z80 and SNA snapshot files.
 ...
 * Also supports SNA files (48K and 128K).
 * 
 * Based on C source code: dbpoke.c, dbscreen.c
 * by Martijn van der Heide
 * 
 * Usage: pokesgd_v006.php?file=path/to/file.Z80&POKES=address,value:address,value
 * Alternative: pokesgd_v006.php?file=path/to/file.Z80 (uses file.POK in POKES/ subdirectory)
 * Also supports: pokesgd_v006.php?file=path/to/file.SNA&POKES=address,value:address,value
 * 
 *
 * Examples :
 * pokesgd_v006.php?file=ROMSMINE/F/FIRELORD.SNA&POKES=34509,0:34984,58  (SNA)
 * pokesgd_v006.php?file=ROMSMINE/F/FIRELORD.Z80&POKES=34509,0:34984,58  (v1.45)
 * pokesgd_v006.php?file=ROMSMINE/L/LASTDUEL.Z80&POKES=37605,0:37672,0:40063,0 (v2.01)
 * pokesgd_v006.php?file=ROMSMINE/L/LIGHT128.Z80&POKES=40750,0:40754,0 (v3 128k)
 * pokesgd_v006.php?file=ROMSMINE/L/LASTVAMP.Z80&POKES=51813,0:51814,0 (v3)
 *
 *
 *
 * @version 0.06c  - Fixed bank/page mapping for 128K Z80 v2/v3 snapshots
 *                 - Added getPageFromBank function for proper page lookup
 *                 - Improved error handling for unsupported banks
 *                 - Corrected offset calculations for different memory pages
 *                 - Changed: POK file entries now ignore bank value and use auto-detection (same as URL parameters) 
 */
...
function getPageFromBank($bank, $is128K, $bankToPageMap)
{
    if ($is128K) {
        if (isset($bankToPageMap[$bank])) {
            return $bankToPageMap[$bank];
        }
        return null;
    } else {
        if ($bank === null || $bank === 8) {
            return convert48KBankToPage($bank);
        }
        if (isset($bankToPageMap[$bank])) {
            return $bankToPageMap[$bank];
        }
...
function applyPokesToMemory(&$memory, $pokes, $is128K, $bankToPageMap = [])
{
    foreach ($pokes as $poke) {
        ...
        $page = getPageFromBank($bank, $is128K, $bankToPageMap);
        
        if ($page === null) {
            if ($is128K) {
                throw new Exception("Memory block for bank $bank not found in 128K snapshot at address $address");
            } else {
                throw new Exception("Memory block for bank $bank not found in 48K snapshot at address $address");
            }
        }
        ...
        if ($is128K) {
            if ($page == 8) {
                ...
                throw new Exception("Unsupported page for 128K: $page at address $address");
            }
...
function parsePOKFile_SNA($pokFilePath) {
    $pokes = [];
...
try {
    ...
    if (empty($file)) {
        throw new Exception("Missing 'file' parameter. Usage: pokesgd_v006.php?file=path/to/file.Z80&POKES=address,value:address,value");
    }
    ...
    if ($extension === 'sna') {
        if (!validateSNAFile_SNA($file)) {
            throw new Exception("Invalid SNA file: $file");
        }
        
        $pokes = [];
        
        if (!empty($pokesString)) {
            $pokes = parsePokes_SNA($pokesString);
        } else {
            $pathInfo = pathinfo($file);
            ...
            if ($pokFilePath) {
                $pokes = parsePOKFile_SNA($pokFilePath);
            } else {
        ...
        $outputFile = generateOutputFilename_SNA($file);
        
        processSNAFile_SNA($file, $outputFile, $pokes);
        ...
        throw new Exception("Unsupported file extension: $extension. Only .Z80 and .SNA files are supported.");
    }
