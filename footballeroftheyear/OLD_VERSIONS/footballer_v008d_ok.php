...
<head>
	...
	Changes:
	v008c_040125 - Next week on match exit, league standings with country/division dropdowns
	v008b_040125 - Country/Division/Team selection from teams.txt, removed position selection
	v008a_040125 - Improved goalkeeper visual feedback in penalty shootout
...
<body>
    <div class="game-container">
        ...
        <div id="mainMenu" class="screen">
            ...
            <div class="goal-cards">
                <div class="goal-card">🎯 Goal Cards: <span id="menuGoalCards">3</span></div>
    ...
    <script>
        ...
        function generateMatchSchedule() {
            ...
            const divTeams = currentDivision.teams.filter(t => t !== player.currentClub);
        ...
        function simulateWeekMatches() {
            ...
            if (matchState.isPlaying && matchState.teamScore > 0) {
                const result = simulateMatchWithPlayerGoals(player.currentClub, playerOpponent, divKey, matchState.teamScore);
                results.push(result);
            } else {
                const playerResult = simulateMatch(player.currentClub, playerOpponent, divKey);
                results.push(playerResult);
            }
            
            for (let i = 0; i < shuffled.length - 1; i += 2) {
                const result = simulateMatch(shuffled[i], shuffled[i + 1], divKey);
                results.push(result);
            }
            ...
            teamsData.countries.forEach(country => {
                Object.keys(country.divisions).forEach(divNum => {
                    ...
                    if (!divisions[otherDivKey]) return;
                    
                    const otherDivTeams = divisions[otherDivKey].teams;
                    if (!otherDivTeams || otherDivTeams.length === 0) return;
                    
                    const otherShuffled = [...otherDivTeams].sort(() => Math.random() - 0.5);
                    
                    for (let i = 0; i < otherShuffled.length - 1; i += 2) {
                        simulateMatch(otherShuffled[i], otherShuffled[i + 1], otherDivKey);
                    }
                });
            });
        ...
        function takePenaltyShot(zone) {
            ...
            const successChance = shooting / 100 + 0.1;
            const goalkeeper = document.getElementById('goalkeeper');
            const ball = document.getElementById('ball');
        ...
        function endSeason() {
            ...
            if (seasonGoals >= promotionThreshold && player.division > 1) {
                ...
                seasonSummary += `🎉 PROMOTED to ${player.divisionName}!\n`;
                seasonSummary += `Salary increased to £${player.weeklySalary}/week`;
                
                const newDivKey = getCurrentDivisionKey();
                const newDivTeams = divisions[newDivKey].teams;
                ...
                seasonSummary += `😔 RELEGATED to ${player.divisionName}\n`;
                ...
                seasonSummary += `You remain in ${player.divisionName} with ${player.currentClub}`;
            }
        ...
        function updateLeagueDisplay() {
            ...
            console.log('Setting header to:', `${countryToView} - Division ${divisionToView} - ${divisionName}`);
            leagueDivisionEl.textContent = `${countryToView} - Division ${divisionToView} - ${divisionName}`;
