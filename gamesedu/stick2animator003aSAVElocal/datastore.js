function Datastore(group, service, proxy){
  this.group = group
  this.service = service?service:"localStorage"
  this.proxy = proxy;
  this.cache = {};
}
...
Datastore.prototype.send = function(p, callback){
  ...
  // Use localStorage instead of remote service
  if(this.service === "localStorage"){
    var storageKey = "datastore_" + this.group + "_" + p.type;
    ...
    if(p.act === "write"){
      ...
      if(data){
        tc.onmessage(JSON.parse(data), callback);
      } else {
        tc.onmessage({error: "Not found"}, callback);
      }
    } else if(p.act === "list"){
      var results = [];
      for(var key in localStorage){
        if(key.startsWith("datastore_" + this.group + "_")){
          results.push(JSON.parse(localStorage[key]));
        }
      }
      tc.onmessage({results: results}, callback);
    }
  } else {
    // Fallback to original remote service
    var param = "act="+encodeURIComponent(p.act)+
